...
public class AuthFavoriteController {
    ...
    @ApiOperation(value = "收藏列表(分页)") @PostMapping("/page")
    public Result<?> page(@RequestBody java.util.Map<String, Object> req) { return biz.page(req); }

    @ApiOperation(value = "收藏列表(全部)") @PostMapping("/list")
    public Result<List<AuthFavoriteResp>> list(@RequestParam(required = false) Integer targetType) {
        return biz.list(targetType);
    }
